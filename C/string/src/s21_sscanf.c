...
void process_int_format(int length, int *width, const char **str, va_list args,
  ...
  if (*width > 0) {
    ...
    buf[i] = '\0';
    *(int *)pval = s21_atoi(buf) * isNegative;
    (*width) = 0;
  } else {
    *(int *)pval = parse_int(length, str) * isNegative;
  }
...
static long double parse_double(const char **str) {
  long double integer_part = 0, fraction_part = 0;
  ...
  long double result = (integer_part + fraction_part) * powl(10, exponent_part);
  return result;
}
...
static float parse_float(const char **str) {
  float integer_part = 0, fraction_part = 0;
  ...
  float result = (integer_part + fraction_part) * powf(10, exponent_part);
  return result;
}
...
void process_x_format(int length, int *width, const char **str, va_list args,
  ...
  if (*width > 0) {
    ...
    if (**format == 'u') {
      if (r) *pval = s21_strtoul(length, buf, 10);
    } else if (**format == 'o') {
      if (r) *pval = s21_strtoul(length, buf, 8);
    } else if (r) {
...
void int_x_width_pars(int length, va_list args, int *read_values,
                      const char **str, int isNegative, int base,
                      const int *width) {
  int r = 1;
  unsigned int *pval = 0;
